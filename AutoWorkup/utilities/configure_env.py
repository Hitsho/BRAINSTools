...
# PATH
append_os_path = [os.path.abspath(os.path.expanduser(os.path.expandvars(item))) for item in append_os_path.split(os.pathsep)]
old_os_path = os.environ['PATH'].split(os.pathsep)
old_os_path.insert(0, os.path.dirname(os.path.abspath(__file__)))
# Move the added items to the front of the path:
for item in list(old_os_path):
    if item in append_os_path:
        old_os_path.remove(item)
os.environ['PATH'] = os.pathsep.join(append_os_path + old_os_path)
